...
def buscar_alunos(busca, page:int=1):
    limite=10
    with Session(engine) as session:
        offset=(page-1)*limite
        query = select(Aluno).where(col(Aluno.nome).contains(busca)).order_by(Aluno.nome).offset(offset).limit(limite+1)
        res=session.exec(query).all()

        flag_proxima=len(res)>limite

        return res[:limite],flag_proxima
    
@app.get("/lista", response_class=HTMLResponse)
def lista(request: Request, busca: str | None='',page:int=1):
    alunos,flag_proxima = buscar_alunos(busca,page)

    return templates.TemplateResponse(request, "lista.html", {"alunos": alunos, "page":page, "flag_proxima":flag_proxima})
